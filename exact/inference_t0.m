function [sample] = inference_t0(inv_pi, ...
                              t, ...
                              n, ...
                              r_0, ...
                              nu_0, ...
...
% t0:
%   if t > n-t0 then the Beta approximation is not used. Instead,
%              ~Beta is computed by integration and slice-sampling is done
%	       for theta_j, j>n-t0 
%              (recommended t0 = 11, and t0-->n-t0 below)
% ngridBeta: 
%   size of grid for numerical integration of ~Beta. recommended = 100
% randseed:
...
if strcmp(resumefilename, '')
    ...
    % Init rho: for SLICE_GIBBS, we need to seed to a reasonable default,
    % which we do by drawing from the approximate BETA_GIBBS prior (init
    % value is never used for BETA_GIBBS)
