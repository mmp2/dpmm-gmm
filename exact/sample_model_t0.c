...
/* Rank t0 after which exact sampling */
unsigned int g_t0;
...
#include "resample_sigmarho_t0.c"
#include "resample_c_t0.c"
...
int initialize(int lhsCount, mxArray* lhs[], int rhsCount, const mxArray* rhs[])
{
	...
	/* t0 and grid */
	g_t0 = saferound(mxGetScalar(rhs[R_T0]));
	g_ngridBeta = saferound(mxGetScalar(rhs[R_NGRIDBETA]));
	...
	/* Create the integration grid on [0, 1) for ~Beta */
	g_grid01 = mxMalloc( g_ngridBeta*sizeof( double ));
	for( i = 0; i < g_ngridBeta; ++i ) {
	  g_grid01[ i ] = (double)i/g_ngridBeta;
	}
...
void mexFunction(int lhsCount, mxArray* lhs[], int rhsCount, const mxArray* rhs[])
{
	...
	/*	fast_print("Entering MEX function sample_model_t0\n");*/
	...
	for (iters = 0; iters < g_iterations; ++iters)
	{
		...
		*randLengthValue += (double)(2 * g_max_t * g_N); 
                  /* for singletons, once in step 1 and once in step 2
		     [including possibility of being called from c? a
		     comment by Harr in v4 that I don't understand] */
		*randLengthValue += (double)(g_sigma_gibbs * g_max_t * g_N/2 * (1+g_rho_slices)); 
		  /* for step 2, non-singleton cluster parameters, sigma
		   and rho sampling. N/2 >= #non-singleton clusters */
		...
		/* RESAMPLE c */
		log_start("resample_c_t0");
		order = randperm(g_N);
		for (i = 0; i < g_N; ++i)
		{
			resample_c_t0(order[i]);
			if ((i + 1) % C_NOTIFICATION_PERIOD == 0)
			{
				mexPrintf("Finished permutation %d\n", i + 1);
				log_end("resample_c_t0");
				log_start("resample_c_t0");
			}
		}
		mxFree(order);
		...
		/* RESAMPLE sigma */
		log_start("resample_sigmarho_t0");
		order = randperm(g_num_valid_clusters);
		for (i = 0; i < g_num_valid_clusters; ++i)
		{
			resample_sigmarho_t0(g_valid_clusters[order[i]]);
			if ((i + 1) % SIGMA_NOTIFICATION_PERIOD == 0)
			{
				mexPrintf("Finished cluster %d\n", i + 1);
				log_end("resample_sigmarho_t0");
				log_start("resample_sigmarho_t0");
			}
		}
		mxFree(order);
	...
	/*	fast_print("Leaving MEX function sample_model_t0\n");*/
}
